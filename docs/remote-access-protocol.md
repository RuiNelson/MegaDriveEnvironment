...
`GET_GAME_UPTIME_FRAMES` returns an unsigned 64-bit count that starts at zero
with each game run and increments once for every physical VSync generated by
the VDP. Turbo mode therefore increases this count at the turbo VSync rate.
