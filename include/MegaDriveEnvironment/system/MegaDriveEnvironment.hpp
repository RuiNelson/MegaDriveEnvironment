 ...
 *    on the CPU thread.
 *
 *  - Games override vSync() and hSync(int) to handle VDP interrupts. New games
 *    should model their frame and scanline work there, rather than manually
 *    polling or dispatching VDP interrupts from the main loop.
 *
 * ```cpp
 ...
 *         }
 *     }
 *     void vSync() override { ... }          // vertical blank
 *     void hSync(int line) override { ... }  // horizontal blank
 * };
 *
...
class MegaDriveEnvironment {
    ...
    /// Complete VSync frames elapsed since the current game run was powered on
    /// or last cold-reset.
    std::uint64_t gameUptimeFrames() const {
        return gameUptimeFrames_.load(std::memory_order_acquire);
    }

    /// Immediately drains pending VDP callbacks. Normal subclasses do not need
    /// to call this: pace() and waitForInterrupt() dispatch them automatically.
    ...
    /// Override to handle vertical blank.
    virtual void vSync() {
    }
    ...
    void dispatchPendingVDPCallbacks() {
        if (vdpCallbacksPending_.load(std::memory_order_acquire)) {
            runVDPInterrupts();
        }
    }

    void notifyVDPCallbackScheduled() {
        vdpCallbacksPending_.store(true, std::memory_order_release);
        interruptGeneration_.fetch_add(1, std::memory_order_release);
        interruptGeneration_.notify_one();
    }

    /// Records one physical VSync generated by the VDP render thread.
    void recordVSync() {
        gameUptimeFrames_.fetch_add(1, std::memory_order_release);
    }
    ...
    std::atomic<std::uint64_t> gameUptimeFrames_{0};
    ...
    std::atomic<bool>            vdpCallbacksPending_{false};
